...
// Control variables
int currentServo = 1;      // Currently selected servo (1, 2, 3, or 4)
int stepSize = 5;          // How much to move each step
...
// Individual servo limits, undecided limits for now
...
void KeyboardController::init() {
    ...
    // Print instructions
    Serial.println("=== KEYBOARD SERVO CONTROL READY ===");
    Serial.println("");
...
bool KeyboardController::update() {
    ...
    if (Serial.available() > 0) {
        ...
        switch (command) {
            ...
            default:
                // Handle arrow key sequences (they send multiple characters)
    ...
    return true;  // Continue running keyboard controller
}

// Helper function to get current servo's position
int* KeyboardController::getCurrentPosition() {
    switch (currentServo) {
...
// Helper function to get current servo's min position
int KeyboardController::getCurrentMin() {
    switch (currentServo) {
...
// Helper function to get current servo's max position
int KeyboardController::getCurrentMax() {
    switch (currentServo) {
...
// Helper function to write to current servo
void KeyboardController::writeToCurrentServo(int position) {
    switch (currentServo) {
